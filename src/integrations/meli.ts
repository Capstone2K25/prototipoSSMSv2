...
export async function getMeliLinkBySku(sku: string) {
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) throw error;
  return data as { meli_item_id: string; meli_variation_id: string | null } | null;
}
...
export async function updateMeliStockBySku(sku: string, newQty: number) {
  ...
  if (link.meli_variation_id) {
    ...
    payload = {
      variations: [{ id: Number(link.meli_variation_id), available_quantity: newQty }]
    };
  } else {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) {
      ...
      if (!res2.ok) throw new Error(`ML update retry failed: ${res2.status} ${await res2.text()}`);
