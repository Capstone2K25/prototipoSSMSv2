...
import { Globe, RefreshCw, CheckCircle, AlertCircle, Repeat } from "lucide-react";
...
export default function Web() {
  ...
  async function fetchProducts() {
    // Leemos desde BD para poder agrupar por familia/talla (igual que en ChannelView de ML)
    const { data, error } = await supabase
      .from("productos")
      .select(`
        id, name, sku, price, stockweb, categoria_id,
  ...
  async function handleSyncAll() {
    try {
      setSyncing(true);
      // Baja catálogo y stocks desde Woo → actualiza tablas locales
      await wooSyncDown();
      await fetchProducts();
  ...
  const connected = !error; // si hay error al cargar, mostramos Desconectado

  // Armamos familias (igual patrón que ChannelView)
  ...
  const columnsForFam = (fam: FamRow) => (fam.tipo === "numerica" ? tallasByTipo.numerica : tallasByTipo.alfanumerica);

  const totalStockWeb = useMemo(() => products.reduce((sum, p) => sum + (p.stockweb || 0), 0), [products]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          ...
          {connected ? (
            <>
              ...
              <p className="text-xs text-neutral-500 mt-1">Última sync: {formatDate(lastSync)}</p>
            </>
          ) : (
            <>
              <p className="text-2xl font-bold text-red-600">Desconectado</p>
              <p className="text-xs text-neutral-500 mt-1">Revisa credenciales de WooCommerce/API.</p>
            </>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <h3 className="text-sm font-semibold text-neutral-600 uppercase mb-2">Productos con stock Web</h3>
        ...
        {/* Stock Web total */}
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <h3 className="text-sm font-semibold text-neutral-600 uppercase mb-2">Stock Web Total</h3>
      ...
      <div className="bg-white rounded-xl shadow-sm border p-6">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b">
                ...
                <th className="text-left  py-3 px-4 text-sm font-semibold">Tallas (Stock Web)</th>
                <th className="text-center py-3 px-4 text-sm font-semibold">Total Web</th>
              </tr>
            </thead>
            <tbody>
              {fams.map((fam, idx) => {
                const cols = columnsForFam(fam);
                const totalFam = cols.reduce((acc, t) => acc + (fam.byTalla[t.id]?.stockweb || 0), 0);
                      ...
                      <div className="overflow-x-auto" style={{ minWidth: 420 }}>
                        ...
                        >
                          ...
                          <div className="text-right pr-2">
                            <span className="px-2 py-1 rounded text-xs font-semibold bg-blue-100 text-blue-700">Stock Web</span>
                          </div>
                          {cols.map((t) => {
        ...
        {/* Acción masiva (igual que tarjeta de acciones del screenshot) */}
